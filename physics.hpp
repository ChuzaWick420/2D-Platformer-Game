...
void apply_gravity(Player& target, Level& tile, float& gravity_val, bool& on_ground) {
	...
	if (index != -1) {
		...
		if (target.on_ground == false) {
			...
			if (check_direction(target, tile.hitboxes[index]) == "Bottom") {
		...
		if (check_direction(target, tile.hitboxes[index]) == "Right") {
			...
			target.player_sprite.setPosition(tile.hitboxes[index].get_position().x - target.get_hitbox().get_size().x + target.player_sprite.getGlobalBounds().width / 2, target.get_position().y);
		...
		if (check_direction(target, tile.hitboxes[index]) == "Left") {
			...
			target.player_sprite.setPosition(tile.hitboxes[index].get_position().x + tile.hitboxes[index].get_size().x + target.player_sprite.getGlobalBounds().width / 2, target.get_position().y);
		}
